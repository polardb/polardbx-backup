...
void LizardTablespace::shutdown() {
  ...
  fil_space_t::s_lizard_space = nullptr;
}
...
bool LizardTablespace::interpret_file() {
  ...
  /** path, and name should have been set in advance */
  ut_ad(path() && name());
...
dberr_t LizardTablespace::check_tablespace_header() {
  /* Only relevant for the lizard tablespace. */
  ut_ad(space_id() == dict_lizard::s_lizard_space_id);
  ...
  for (int retry = 0; retry < 2; ++retry) {
    err = it->validate_first_page(it->m_space_id, nullptr, false);
