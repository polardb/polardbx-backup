...
run_cmd xtrabackup --no-timestamp --stream=xbstream --rds-table-level $topdir/tmp_backup \
...
run_cmd xtrabackup --prepare --apply-log-only --rds-table-level $topdir/backup
...
run_cmd xtrabackup --prepare --rds-table-level $topdir/backup
...
if ! egrep -q "file for 'sakila/film.*it will be removed from the data dictionary" $OUTFILE; then
    die "sakila/film.* should be removed, but actually not."
fi

start_server

num=$(${MYSQL} ${MYSQL_ARGS} -BNe "SELECT count(*) FROM INFORMATION_SCHEMA.INNODB_SYS_TABLES WHERE NAME LIKE 'sakila/film%'")
